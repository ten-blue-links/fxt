...
double
avg_term_len(indri::index::Index* const idx,
...
double
entropy(indri::index::Index* const idx,
        const indri::utility::greedy_vector<lemur::api::TERMID_T>& terms)
{
  double ent = 0.0;
...
double
stop_cover(const std::unordered_map<lemur::api::TERMID_T, int>& stopwords,
           const indri::utility::greedy_vector<lemur::api::TERMID_T>& terms)
{
  double cov = 0.0;
...
double
frac_stop(const std::unordered_map<lemur::api::TERMID_T, int>& stopwords,
          const indri::utility::greedy_vector<lemur::api::TERMID_T>& terms)
{
  int stop = 0;
  int nonstop = 0;
...
double
frac_anchor_text(indri::index::Index* const idx,
                 const indri::utility::greedy_vector<indri::index::FieldExtent>& list)
{
  double ret = 0.0;
  int a, b;
...
double
frac_vis_text(const score& s, const size_t bytes)
{
  double ret = 0.0;
...
double
frac_td_text(indri::index::Index* const idx,
             const indri::utility::greedy_vector<indri::index::FieldExtent>& list)
{
  double ret = 0.0;
  int a, b;
...
double
frac_table_text(indri::index::Index* const idx,
                const indri::utility::greedy_vector<indri::index::FieldExtent>& list)
{
  double ret = 0.0;
  int a, b;
...
void
set_url_lendep(score& s, const std::string& url)
{
  size_t depth = 0;
...
main(int argc, char **argv)
{
  ...
  while (!it->finished()) {
    ...
    std::cout << doc_name << "," << s.len << ","
              << s.title_len << "," << s.avg_term_len << ","
              << s.entropy << "," << s.stop_cover << ","
              << s.frac_stop << "," << s.visterm_len << ","
              << s.frac_anchor_text << "," << s.frac_vis_text << ","
              << s.frac_table_text << "," << s.frac_td_text << ","
              << s.url_len << "," << s.url_depth
              << std::endl;
