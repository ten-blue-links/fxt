...
void gamma1(query_t &q, std::unordered_map<std::string, term_t> &termmap,
  ...
  for (i = 0; i < q.len; i++) {
    ...
    idf = (std::log2(total_docs + 0.5) / ct->second.cdf) /
          std::log2(total_docs + 1);
    sum += idf;
    sum_sqrs += idf * idf;
  }
 ...
 * SPIRE 2004.
 */
double gamma2(query_t &q, std::unordered_map<std::string, term_t> &termmap,
              std::set<std::string> &set) {
  size_t i;
 ...
 * SPIRE 2004.
 */
double scs_score(query_t &q, std::unordered_map<std::string, size_t> &qmap,
 ...
 * the query scope.
 */
double query_scope(query_t &q, std::unordered_map<std::string, term_t> &termmap,
                   std::set<std::string> &set) {
  size_t i;
  uint64_t sum_cdf = 0;
 ...
 */
void avidf(query_t &q, std::unordered_map<std::string, term_t> &termmap,
           std::set<std::string> &set) {
  size_t i;
  ...
  for (i = 0; i < q.len; i++) {
    ...
    if (is_stopword(set, term)) {
      idf_full += (std::log2(total_docs + 0.5) / ct->second.cdf) /
                  std::log2(total_docs + 1);
      continue;
    }
 ...
 */
void avictf(query_t &q, std::unordered_map<std::string, term_t> &termmap,
            std::set<std::string> &set) {
  size_t i;
  ...
  for (i = 0; i < q.len; i++) {
    ...
    if (is_stopword(set, term)) {
      ictf_full += (std::log2(total_terms + 0.5) / ct->second.cf) /
                   std::log2(total_terms + 1);
      continue;
    }

    ictf += (std::log2(total_terms + 0.5) / ct->second.cf) /
            std::log2(total_terms + 1);
  }
