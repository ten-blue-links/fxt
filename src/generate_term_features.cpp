...
int main(int argc, char **argv) {
  ...
  app.add_option("-i,--inverted-index", inverted_index_file,
                 "Inverted index filename")
      ->required();
  app.add_option("-d,--doc-lens", doc_lens_file, "Document lens filename")
      ->required();
...

  {
    ...
    std::cerr << "Loaded " << inverted_index_file << " in " << load_time.count()
              << " ms" << std::endl;
  }
  {
    auto start = clock::now();
    ...
    auto load_time =
        std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);
    std::cerr << "Loaded " << doc_lens_file << " in " << load_time.count()
              << " ms" << std::endl;
  }
  ...
  for (auto &&pl : inv_idx) {
    ...
    if (pl.length() >= 4) {
      ...
      compute_be_stats(feature, doc_lens, list, ndocs, avg_dlen,
                       pl.term_count(), be_max);
      compute_dph_stats(feature, doc_lens, list, ndocs, avg_dlen,
                        pl.term_count(), dph_max);
      compute_dfr_stats(feature, doc_lens, list, ndocs, avg_dlen,
                        pl.term_count(), dfr_max);
      outfile << feature;
      freq++;
    }
    done++;
