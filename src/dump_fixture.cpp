...
void dump_lexicon(Lexicon& lex) {
  ...
  for (size_t i = 1; i < lex.length(); ++i) {
    ...
    auto c = t.get_counts();
    std::cout << "{" << std::endl;
    std::cout << "Counts c(" << c.document_count << ", " << c.term_count << ");"
              << std::endl;
    std::cout << "FieldCounts fc;" << std::endl;
    for (auto& e : t.get_field_counts()) {
      std::cout << "fc.insert(std::make_pair(" << e.first << ", "
                << "Counts(" << e.second.document_count << ", "
                << e.second.term_count << ")));" << std::endl;
    }
...
void dump_doc(Document& d, size_t id) {
  ...
  std::cout << "const std::map<uint16_t, Field> doc_field_stats_" << id
            << " = {};" << std::endl;

  std::cout << "{" << std::endl;
  std::cout << "Document doc_" << id << "(" << id << ");" << std::endl;
  std::cout << "doc_" << id << ".set_fields(doc_fields_" << id << ");"
            << std::endl;
  std::cout << "doc_" << id << ".set_terms(doc_terms_" << id << ");"
            << std::endl;
  std::cout << "doc_" << id << ".set_freq(doc_freqs_" << id << ");"
            << std::endl;
...
void dump_posting(PostingList& p, size_t id) {
  ...
  std::cout << "{" << std::endl;
  std::cout << "PostingList pl_" << id << "(\"" << p.term() << "\", "
            << p.term_count() << ");" << std::endl;
...
int main(int argc, char** argv) {
  ...
  std::cerr << "Loaded " << fwdpath << " in " << load_time.count() << " ms"
            << std::endl;
  ...
  load_time =
      std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);
  std::cerr << "Loaded " << lexpath << " in " << load_time.count() << " ms"
            << std::endl;
  ...
  load_time =
      std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);
  std::cerr << "Loaded " << invpath << " in " << load_time.count() << " ms"
            << std::endl;

  std::cout << "#include \"fxt/forward_index.hpp\"" << std::endl;
  std::cout << "#include \"fxt/inverted_index.hpp\"" << std::endl;
  std::cout << "#include \"fxt/lexicon.hpp\"" << std::endl << std::endl;
  std::cout << "namespace fixture {" << std::endl << std::endl;
  std::cout << "Lexicon stub_lexicon() {" << std::endl;
  std::cout << "Lexicon lexicon(Counts(" << lexicon.document_count() << ", "
            << lexicon.term_count() << "));" << std::endl;
