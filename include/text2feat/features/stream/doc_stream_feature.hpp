   ...
   public:
    void compute(query_train &qry, doc_entry &doc, Document &doc_idx, FieldIdMap &field_id_map) {
        ...
        if (body_tf) {
            ...
            doc.mean_stream_len_body =
                (double)((double)doc_idx.field_len(body_id) / doc_idx.tag_count(body_id)) / body_tf;
            doc.variance_stream_len_body =
                (((double)doc_idx.field_len_sum_sqrs(body_id) / doc_idx.field_len(body_id)) -
                 ((double)doc_idx.field_len(body_id) / doc_idx.tag_count(body_id)) *
                     ((double)doc_idx.field_len(body_id) / doc_idx.tag_count(body_id))) /
                body_tf;
        }
        if (title_tf) {
            ...
            doc.mean_stream_len_title =
                (double)((double)doc_idx.field_len(title_id) / doc_idx.tag_count(title_id)) /
                title_tf;
            doc.variance_stream_len_title =
                (((double)doc_idx.field_len_sum_sqrs(title_id) / doc_idx.field_len(title_id)) -
                 ((double)doc_idx.field_len(title_id) / doc_idx.tag_count(title_id)) *
                     ((double)doc_idx.field_len(title_id) / doc_idx.tag_count(title_id))) /
                title_tf;
        }
        if (heading_tf) {
            doc.sum_stream_len_heading = (double)doc_idx.field_len(heading_id) / heading_tf;
            doc.min_stream_len_heading = (double)doc_idx.field_min_len(heading_id) / heading_tf;
            doc.max_stream_len_heading = (double)doc_idx.field_max_len(heading_id) / heading_tf;
            doc.mean_stream_len_heading =
                (double)((double)doc_idx.field_len(heading_id) / doc_idx.tag_count(heading_id)) /
                heading_tf;
            doc.variance_stream_len_heading =
                (((double)doc_idx.field_len_sum_sqrs(heading_id) / doc_idx.field_len(heading_id)) -
                 ((double)doc_idx.field_len(heading_id) / doc_idx.tag_count(heading_id)) *
                     ((double)doc_idx.field_len(heading_id) / doc_idx.tag_count(heading_id))) /
                heading_tf;
        }
        if (inlink_tf) {
            doc.sum_stream_len_inlink = (double)doc_idx.field_len(inlink_id) / inlink_tf;
            doc.min_stream_len_inlink = (double)doc_idx.field_min_len(inlink_id) / inlink_tf;
            doc.max_stream_len_inlink = (double)doc_idx.field_max_len(inlink_id) / inlink_tf;
            doc.mean_stream_len_inlink =
                (double)((double)doc_idx.field_len(inlink_id) / doc_idx.tag_count(inlink_id)) /
                inlink_tf;
            doc.variance_stream_len_inlink =
                (((double)doc_idx.field_len_sum_sqrs(inlink_id) / doc_idx.field_len(inlink_id)) -
                 ((double)doc_idx.field_len(inlink_id) / doc_idx.tag_count(inlink_id)) *
                     ((double)doc_idx.field_len(inlink_id) / doc_idx.tag_count(inlink_id))) /
                inlink_tf;
        }
        if (a_tf) {
            ...
            doc.mean_stream_len_a =
                (double)((double)doc_idx.field_len(a_id) / doc_idx.tag_count(a_id)) / a_tf;
