...
class WScanner {
 public:
  ...
  std::vector<std::pair<lemur::api::DOCID_T, uint64_t>> window_count(
      std::vector<indri::index::DocListIterator *> &doc_iters,
      size_t min_term) {
    _collection_cnt = 0;
    ...
    lemur::api::DOCID_T curr_doc =
        doc_iters[min_term]->currentEntry()->document;
    lemur::api::DOCID_T max_doc =
        curr_doc;                        //!< always keep current largest doc
